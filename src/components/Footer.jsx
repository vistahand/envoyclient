...
const Footer = () => {
  ...
  return (
    <section
      className="relative w-full md:min-h-[300px] ss:min-h-[600px] 
    min-h-[700px] flex items-center flex-col md:pt-0 ss:pt-0 pt-6"
    >
      <div
        className="flex items-center w-full relative md:pb-10 ss:pb-8
        pb-6"
      >
        <div
        ...
        >
          <div className="flex flex-col md:gap-5 ss:gap-6 gap-5">
            ...
            <p
              className="text-main2 md:text-[14px] ss:text-[16px]
                    text-[14px] md:leading-[21px] ss:leading-[23px] leading-[20px]
                    font-medium tracking-tight md:max-w-[45ch]"
            >
              Envoy Angel Shipping and Logistics is a trusted provider of
              seamless, secure shipping solutions between Ireland and Nigeria.
              Simplifying the complexities of international shipping, one
              delivery at a time.
            </p>
              ...
              {socialMedia.map((social, index) => (
                ...
                >
                  {React.createElement(social.image, {
                    className: `${
                      social.id === "facebook"
                        ? "text-primary md:w-7 ss:w-8 w-7 h-auto"
                        : "md:w-4 ss:w-5 w-4 h-auto text-white"
                    }`,
                  })}
                </a>
              ))}
          ...
          <div
            className="w-full flex flex-col md:flex-row md:mt-0 ss:mt-10
                mt-8 md:grid-cols-5 ss:grid-cols-3 grid-cols-2 md:gap-5"
          >
            {footerLinks.map((footerLink, index) => (
              <div
                key={index}
                className="flex flex-col md:my-3 ss:my-3
                        my-3 w-full"
              >
                <h4
                  className={`font-bold md:text-[15px] ss:text-[18px] 
                            text-[14px] text-primary tracking-tight
                            ${
                              index !== footerLinks.length - 1
                                ? "md:mr-14 ss:mr-8 mr-8"
                                : "mr-12"
                            }`}
                >
                  {footerLink.title}
                ...
                <ul
                  className="list-none mt-3 w-full 
                  ...
                  {footerLink.links.map((Link, index) => (
                    <div key={Link.name}>
                      {Link.name === "Track Shipment" ? (
                        ...
                        >
                          <li
                            className={`md:text-[14px] ss:text-[16px] grow2 tracking-tight
      ...
      >
        <div className="flex md:items-center ss:items-center">
          <BiCopyright
            className="sm:mr-1 mr-1 md:text-[16px] 
                ss:text-[18px] text-[15px] mt-1 text-main4"
          />

          <p
            className="md:text-[13px] ss:text-[15px] text-[12px] 
