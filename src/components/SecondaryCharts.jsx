...
const SecondaryCharts = () => {
  ...
  const fetchAnalyticsChartsData = async () => {
    ...
    try {
      ...
      setAnalyticsData({
        deliveryTimeData: isEmptyDataArray(data.deliveryTimeData) ? FALLBACK_DATA.deliveryTimeData : data.deliveryTimeData,
        topDestinationsData: isEmptyDataArray(data.topDestinationsData) ? FALLBACK_DATA.topDestinationsData : data.topDestinationsData,
        carrierData: isEmptyDataArray(data.carrierData) ? FALLBACK_DATA.carrierData : data.carrierData,
        satisfactionData: isEmptyDataArray(data.satisfactionData) ? FALLBACK_DATA.satisfactionData : data.satisfactionData
      });
    } catch (err) {
  ...
  const isEmptyDataArray = (dataArray) => {
    if (!dataArray || !Array.isArray(dataArray) || dataArray.length === 0) return true;
  ...
  const NoDataDisplay = () => (
    <div className="flex h-full items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-gray-400 text-xs mt-1">Using sample data for demonstration</p>
      </div>
    </div>
  );
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6">
      ...
      <div className="bg-white p-3 sm:p-6 rounded-xl shadow-sm relative">
        ...
        <div className="flex justify-between items-center mb-4">
          <h2 className="font-semibold text-sm sm:text-base text-gray-700">Delivery Time Distribution</h2>
          {isEmptyDataArray(analyticsData.deliveryTimeData) && (
            <span className="bg-yellow-100 text-yellow-800 text-xs px-2 py-0.5 rounded">Sample Data</span>
          )}
        </div>
      ...
      <div className="bg-white p-3 sm:p-6 rounded-xl shadow-sm relative">
        ...
        <div className="flex justify-between items-center mb-4">
          <h2 className="font-semibold text-sm sm:text-base text-gray-700">Top Shipment Destinations</h2>
      ...
      <div className="bg-white p-3 sm:p-6 rounded-xl shadow-sm relative">
        ...
        <div className="flex justify-between items-center mb-4">
          <h2 className="font-semibold text-sm sm:text-base text-gray-700">Carrier Performance</h2>
