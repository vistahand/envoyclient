...
const PickupModal = ({ onClose, values, onPrevious }) => {
  ...
  const handlePrevious = () => {
    if (onPrevious && typeof onPrevious === 'function') {
      ...
      // Fallback if no onPrevious function provided
      onClose();
    }
  };
...
const formik = useFormik({
  ...
  onSubmit: async (formValues) => {
    ...
    try {
      ...
      if (response && response.success === true && response.data && response.data.shipment && response.data.shipment._id) {
        alert("Pickup information updated successfully!");
        onClose(); // Close the modal after successful update
      } else {
        // Handle failed but returned response
        console.error("API returned unsuccessful response:", response);
        const errorMessage = response?.message || response?.error || 'Unable to update pickup location';
      ...
      if (error.response) {
        ...
        const errorMessage = error.response.data?.message || error.response.data?.error || 'Server rejected the request';
        alert(`Update Failed: ${errorMessage}`);
      } else if (error.request) {
        // Network error - request was made but no response received
        console.error("No response received:", error.request);
        alert("Network Error: No response from server. Please check your connection and try again.");
      } else if (error.message === 'Invalid response from server') {
        // Our custom thrown error for invalid response format
        alert("Server returned an invalid response format. Please try again or contact support.");
      } else {
