...
const calculateShippingCost = async (shipmentDetails) => {
  try {
    ...
    // Ensure shipment details are provided
    if (!shipmentDetails || Object.keys(shipmentDetails).length === 0) {
      throw new Error('Shipment details are required to calculate cost');
    }
    ...
    if (!response?.data?.cost) {
      throw new Error('Invalid response from server while calculating shipping cost');
    }
