...
const CareerPage = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object().shape({
      ...
      mode: Yup.string()
        .oneOf(["remote", "hybrid", "physical"], "Invalid job mode")
        ...
        if (isEditing) {
          ...
          setCareers((prevCareers) => {
            ...
            return prevCareers.map((job) =>
              job.id === currentJobId || job._id === currentJobId
                ? { ...job, ...submitValues }
                : job
            );
          });
  ...
  const formFields = [
    ...
    { name: "amount", label: "Base Salary (Number only)", type: "number" },
    {
      ...
      label: "Application URL (Link for applicants)",
      type: "url",
    },
  ...
  const updateJobStatus = async (newStatus) => {
    ...
    try {
      ...
      await api.put(
        `/api/admin/job/${selectedJob.id || selectedJob._id}/status`,
        { status: newStatus }
      );
      ...
      setCareers((prevCareers) => {
        if (!Array.isArray(prevCareers)) return [];
        return prevCareers.map((job) =>
          job.id === selectedJob.id || job._id === selectedJob._id
  ...
  const getStatusLabel = (status) => {
    switch (status) {
      case "draft":
        return (
          <span className="bg-gray-300 text-gray-800 text-xs px-2 py-1 rounded-full font-medium">
            Draft
          </span>
        );
      case "published":
        ...
        <div className="bg-white rounded-lg shadow-sm border border-main6/30 overflow-hidden">
          <div className="p-6">
            ...
            {isLoading && (!Array.isArray(careers) || careers.length === 0) ? (
              <div className="py-10 text-center text-main4">
                Loading job listings...
              </div>
            ) : !Array.isArray(careers) ? (
              <div className="py-10 text-center text-main4">
                Error loading job listings. Please refresh the page.
              </div>
            ) : careers.length === 0 ? (
              <div className="py-10 text-center text-main4">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50 text-left">
                      <th className="py-3 px-4 text-[14px] font-semibold text-main4">
                        Title
                      </th>
                      <th className="py-3 px-4 text-[14px] font-semibold text-main4">
                        Location
                  ...
                  <tbody>
                    {Array.isArray(careers) &&
                      careers.map((job) => (
                        <tr
                          key={job.id || job._id}
                          className="border-t border-main6/20"
                        >
                          ...
                          <td className="py-4 px-4 text-[14px] capitalize">
                            {job.mode}
                          </td>
                          <td className="py-4 px-4 text-[14px]">
                          ...
                          <td className="py-4 px-4 text-[14px]">
                            {getActiveLabel(
                              job.active !== undefined ? job.active : true
                            )}
                          </td>
                          <td className="py-4 px-4 text-[14px]">
                            {job.expiryDate ? (
      ...
      >
        <div className="mt-2">
          <p className="text-sm text-gray-600 mb-4">
            Current status:{" "}
            {selectedJob && getStatusLabel(selectedJob.status || "draft")}
          </p>

          <div className="border-t border-gray-200 pt-4">
            <p className="text-sm text-gray-500">
              Changing the status will update the visibility and availability of
              this job posting.
            </p>
            <ul className="mt-2 text-xs text-gray-600 space-y-1">
              <li>
                <strong>Filled:</strong> Applications to this job posting is
                currently maxxed out
              </li>
              <li>
                <strong>Closed:</strong> Job posting is no longer accepting
                applications
              </li>
            </ul>
          </div>
